	...
	<script type="text/javascript">
		$(function() {
			...
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/workflow/runtime/user/tobe.action',
				idField : 'id',//唯一标识列
				sortName : 'createTime',
				sortOrder : 'desc',
				remoteSort : true,
				...
				columns : [ [ {
					...
					rowspan : 2,	
					formatter : function(value, rec) {
						return '<span><a href="#" onclick="editVO(\'' + rec.instanceId + '\');">办理</a>'+
						'</span>';
					}
				} ] ],
				pagination : true,
		...
		function editVO(id){
			window.location.href='<%=request.getContextPath()%>/app/workflow/runtime/run.action?instanceId='+ id;
			return false;
		}
	</script>
</head>
