...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
			...
			$('#queryTable').datagrid({
				...
				onAfterEdit : function (rowIndex, rowData, changes) {
					...
					if (rows.length > 0) {
						...
						$.ajax({
							url : '<%=request.getContextPath()%>/app/workflow/item/ajaxupdate.action', 
							type : 'post',         
							dataType : 'json',     
							data : rowData,         
				...
				toolbar : [ {
					...
					iconCls : 'icon-add',
					handler : function() {
						window.location.href='<%=request.getContextPath()%>/app/workflow/item/add.jsp?flowId=<%=flowId%>';
					...
					handler : function() {
						...
						$.messager.confirm('确认删除项', '确认删除该选项', function(result){
							if (result){
								window.location.href='<%=request.getContextPath()%>/app/workflow/item/delete.action?flowId=<%=flowId%>&ids='+ids.join('__');
							}
						});
						return false;
					}
					...
					handler : function() {
						...
						$.messager.confirm('确认互换项', '确认互换该选项', function(result){
							if (result){
								window.location.href='<%=request.getContextPath()%>/app/workflow/item/change.action?flowId=<%=flowId%>&first='+first.id+'&second='+second.id;
							}
						});
						return false;
					}
					...
					iconCls : 'icon-edit',
					handler : function() {
						window.location.href='<%=request.getContextPath()%>/app/workflow/item/getsql.action?flowId=<%=flowId%>';
						return false;
					}
				}, '-', {
					text:'保存',
		...
		function deleteVO(id){
			$.messager.confirm('确认删除项', '确认删除该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/workflow/item/delete.action?flowId=<%=flowId%>&ids=' + id;
				}
			});
			return false;
		}
		...
		function editVO(id){
			window.location.href='<%=request.getContextPath()%>/app/workflow/item/query.action?flowId=<%=flowId%>&id='+ id;
			return false;
		}
	</script>
</head>
